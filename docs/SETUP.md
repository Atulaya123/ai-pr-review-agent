...
Real output from this repo's Tiger Cloud instance, current hybrid retrieval
(vector + full-text fusion, `hybrid_search_chunk_symbols`) + Gemini embeddings:

```
=== Retrieval metrics (Recall@3, MRR) ===
  recall@3=1.00  top1=modular_monolith_dependency_rule          'Can backend/agents/ import directly from backend/job_qu'
...
=== Generation metrics (LLM-as-judge: Faithfulness, Relevance) ===
  ...
  [ungrounded_finding_no_context] supported=False (OK)  relevant=False (UNEXPECTED)
...
Reading this: Recall@3 is 1.00 on every query and MRR is a perfect 1.000 —
every query's top-ranked result is the correct chunk. That's specific to this
small six-invariant corpus and this embedding model, not a general claim;
re-running the same test set against local Ollama embeddings previously
measured MRR=0.889 on the same database (still perfect recall, just not
always rank 1) — a real, reproducible before/after, not a one-off. The
generation numbers aren't meant to look good — `GENERATION_TEST_SET`
(`backend/evaluation/dataset.py`) is a deliberately mixed fixture (one
grounded finding, one ungrounded, one hallucinated), so ~0.33 on both is the
harness correctly scoring a bad test set as bad, not a real quality number
for the live pipeline. These two also drift slightly run-to-run (e.g.
relevance has landed at both 0.33 and 0.67 across runs) since LLM-as-judge is
itself a live LLM call, not a deterministic lookup — expected, not a bug.
Run it against real findings pulled from `finding_records` to get
